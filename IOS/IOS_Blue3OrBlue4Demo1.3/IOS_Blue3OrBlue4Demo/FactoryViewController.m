...
- (void)viewDidLoad {
    ...
    [[Blue3OrBlue4Manager shareInstance] configureBlueNames:_deviceNames ableDeviceSum:_devSum];
    //连接蓝牙成功回调
     __weak FactoryViewController *weakSelf = self;
     [Blue3OrBlue4Manager shareInstance].blueConBlock = ^(NSString *markKey) {
     ...
     //蓝牙断开回调
     [Blue3OrBlue4Manager shareInstance].blueDisBlock =   ^(NSString *markKey){
         if ([markKey isEqualToString:@"1"]) {
             //判断连接的设备
             NSLog(@"A设备 蓝牙 断开");
             weakSelf.ALabl.text = @"";
     ...
     [Blue3OrBlue4Manager shareInstance].hzlblueDataBlock_B = ^(HZLBlueData *blueData, BlueType conBT, BOOL isFalseCon) {
         if (conBT == BlueType_Pro) {
             if (blueData.bleDataType == BLEMIND) {
     ...
     [Blue3OrBlue4Manager shareInstance].hzlblueDataBlock_C = ^(HZLBlueData *blueData, BlueType conBT, BOOL isFalseCon) {
         if (conBT == BlueType_Pro) {
             if (blueData.bleDataType == BLEMIND) {
     ...
     [Blue3OrBlue4Manager shareInstance].hzlblueDataBlock_D = ^(HZLBlueData *blueData, BlueType conBT, BOOL isFalseCon) {
         if (conBT == BlueType_Pro) {
             if (blueData.bleDataType == BLEMIND) {
     ...
     [Blue3OrBlue4Manager shareInstance].hzlblueDataBlock_E = ^(HZLBlueData *blueData, BlueType conBT, BOOL isFalseCon) {
         if (conBT == BlueType_Pro) {
             if (blueData.bleDataType == BLEMIND) {
     ...
     [Blue3OrBlue4Manager shareInstance].hzlblueDataBlock_F = ^(HZLBlueData *blueData, BlueType conBT, BOOL isFalseCon) {
         if (conBT == BlueType_Pro) {
             if (blueData.bleDataType == BLEMIND) {
...
- (IBAction)testA:(id)sender {
    ...
    if (btn.selected) {
        ...
        [[Blue3OrBlue4Manager shareInstance] testAFalseCon:YES];
    }else{
        [btn setTitle:@"当前不测试A假连接" forState:UIControlStateNormal];
        [[Blue3OrBlue4Manager shareInstance] testAFalseCon:NO];
    }
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[Blue3OrBlue4Manager shareInstance] setTestToZero];
